...
- **我已经安装了 UiAutomator2，为什么仍然弹窗**：电脑上通过 npm 安装的 UiAutomator2 驱动，和手机上的两个辅助 APK 不是同一个东西。手机必须同时保留 `io.appium.uiautomator2.server` 与 `io.appium.uiautomator2.server.test`；首次确认两个弹窗后，程序才会在后续会话跳过重复安装。
